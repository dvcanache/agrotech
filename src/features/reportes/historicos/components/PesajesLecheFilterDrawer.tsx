...
export interface PesajesLecheFilterValues {
  lactanciaMin?: number;
  lactanciaMax?: number;
  desde?: string;
  hasta?: string;
...
interface PesajesLecheFilterDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  filters: PesajesLecheFilterValues;
  onFilterChange: (newFilters: PesajesLecheFilterValues) => void;
  onReset: () => void;
}

const ALL_TIPOS_PESAJE: ('Inicio' | 'Ordeño' | 'Secado')[] = ['Inicio', 'Ordeño', 'Secado'];
...
export const PesajesLecheFilterDrawer: React.FC<PesajesLecheFilterDrawerProps> = ({
  isOpen,
  onClose,
  filters,
  onFilterChange,
  ...
  return (
    <ReportFilterDrawer
      isOpen={isOpen}
      onClose={onClose}
      title="Filtros: Pesajes de Leche"
      onClear={onReset}
    >
      {/* Rango de fechas */}
        ...
        <div className="filter-checkbox-list">
          {ALL_TIPOS_PESAJE.map(tp => (
            <label key={tp} className="filter-checkbox-label">
              <input
                type="checkbox"
                checked={filters.tipoPesaje.includes(tp)}
                onChange={() =>
                  onFilterChange({
                    ...filters,
                    tipoPesaje: toggleArrayItem(filters.tipoPesaje, tp)
                  })
                }
              />
              <span>{tp}</span>
